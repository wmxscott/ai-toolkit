---
name: setup
description: Sets Claude Code's theme to the theme-sync theme, which follows the macOS light/dark appearance through theme-monitor. Run once after installing the plugin.
...
2. Act on its exit status:
   ...
   - **4**: theme-monitor isn't running. Nothing has changed. Show the user the install command from the output and tell them to run setup again once theme-monitor is running. Don't run the install for them.
