...
def test_session_start_hook_runs_on_every_start():
    ...
    assert hook["command"] == 'bash "${CLAUDE_PLUGIN_ROOT}/scripts/session_start.sh"'
...
def test_setup_skill_runs_the_bundled_script_and_preapproves_it():
    ...
    command = "python3 ${CLAUDE_PLUGIN_ROOT}/scripts/setup_theme_sync.py"
...
def test_no_fixed_install_paths():
    files = shipped_files()
    assert PLUGIN / "scripts" / "theme_sync.py" in files
    for path in files:
        if path.is_file():
            text = path.read_text()
